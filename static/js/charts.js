...
function renderAutarky(data) {
  destroy('autarky');
  const ctx = document.getElementById('chart-autarky');
  if (!ctx) return;
  const periods = data.grid?.periods || [];
  if (!_hideIfEmpty(ctx, periods.length > 0)) return;
  ...
  charts.autarky = new Chart(ctx, {
    type: 'line',
    data: {
      labels: periods.map(p => p.label),
      datasets: [
        { label: window.T?.chart_autarky_self_sufficiency || 'Self-sufficiency (share of consumption)', data: autarky, borderColor: CHART_COLORS.good, backgroundColor: 'rgba(76,175,80,0.12)', fill: true, tension: 0.25 },
        { label: window.T?.chart_autarky_self_consumption || 'Self-consumption (share of production)', data: periods.map(p => p.self_consumption_pct), borderColor: CHART_COLORS.actualLine, tension: 0.25, fill: false },
      ],
    },
    options: {
